...
conjugation parameters ρ that define a variational prior Q_ρ(z) in the latent
exponential family, and minimize D(Q_ρ || Q_Z) as a regularizer.

For a harmonium with joint distribution:
    p(x, z) ∝ exp(θ_X · s_X(x) + θ_Z · s_Z(z) + s_X(x) · Θ_XZ · s_Z(z))

The variational prior is:
    Q_ρ(z) = exp((θ_Z + ρ) · s_Z(z) - ψ_Z(θ_Z + ρ))

where ρ are the learned conjugation parameters.

The combined training objective is:
    L = α_cd · D(P_X || Q_X) + α_conj · D(Q_ρ || Q_Z)

All gradient computations for D(Q_ρ || Q_Z) use expectations under Q_ρ, which is
easy to sample from since it's in the latent exponential family.

Key insight: When Θ_XZ = 0 and ρ = 0, the conjugation is exact (D(Q_ρ || Q_Z) = 0).
"""
...
class ConjugatedHarmoniumState(NamedTuple):
    ...
    Attributes:
        harmonium_params: Standard harmonium parameters [θ_X, Θ_XZ, θ_Z]
        conjugation_params: ρ - same shape as lat_man.dim
    """
...
class ApproximateConjugationConfig:
    ...
    Attributes:
        ...
        alpha_conj: Weight on conjugation objective D(Q_ρ || Q_Z)
        n_conj_samples: Number of samples from Q_ρ for gradient estimation
        n_gibbs_conj: Number of Gibbs steps to approximate E_p from Q_ρ samples
    """
...
def initialize_conjugated(
    ...
    """Initialize with Θ_XZ ≈ 0, ρ = 0 for exact conjugation at start.

    With Θ_XZ = 0 and ρ = 0:
    - The conjugation equation is trivially satisfied: ψ_X(θ_X) = const
    - Q_ρ(z) = Q_Z(z) exactly (both are exp family with params θ_Z)
    - D(Q_ρ || Q_Z) = 0
    """
...
def make_conjugation_gradient_fn(
    ...
    def conjugation_gradient(
        ...
        """Compute ∂_ρ D(Q_ρ || Q_Z) = Cov_{Q_ρ}(f(z), s_Z(z))."""
...
def make_harmonium_conjugation_gradient_fn(
    ...
    Computes the gradient of D(Q_ρ || Q_Z) w.r.t. harmonium params.

    The gradient is: E_p[stats] - E_{Q_ρ}[conditional_stats]

    where E_p is approximated by running Gibbs chains initialized from Q_ρ samples.
    ...
    def harmonium_conjugation_gradient(
        ...
        """Gradients of D(Q_ρ || Q_Z) w.r.t. harmonium params."""
        # Sample z from Q_ρ (variational prior)
        key, sample_key = jax.random.split(key)
        _, _, lat_params = model.split_coords(harmonium_params)
        prior_params = lat_params + conjugation_params
        z_samples = model.pst_man.sample(sample_key, prior_params, n_samples)

        # Run Gibbs chains initialized from Q_ρ samples to approximate E_p
        ...
        # E_{Q_ρ}[conditional expectations E[x|z]]
        ...
        # Gradient: E_p - E_{Q_ρ}[conditional]
...
def make_estimate_conjugation_error_fn(
    ...
    def estimate_error(
        ...
        """Estimate conjugation error as Var[f(z)] under Q_ρ."""
        _, _, lat_params = model.split_coords(harmonium_params)
        prior_params = lat_params + conjugation_params
        z_samples = model.pst_man.sample(key, prior_params, n_samples)
