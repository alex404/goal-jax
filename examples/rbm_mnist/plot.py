...
def main():
    ...
    # 1.4 ||ρ|| (conjugation parameter norm)
    ax = axes[0, 3]
    ...
    for i in range(n_samples):
        ...
        comparison[28:, i * 28 : (i + 1) * 28] = np.array(
            results["baseline_reconstructed_images"][i]
        ).reshape(28, 28)
    ...
    for i in range(n_samples):
        ...
        comparison[28:, i * 28 : (i + 1) * 28] = np.array(
            results["conj_reconstructed_images"][i]
        ).reshape(28, 28)
    ax.imshow(comparison, cmap="gray")
    ...
    for i in range(n_gen):
        generated[:, i * 28 : (i + 1) * 28] = np.array(
            results["baseline_generated_samples"][i]
        ).reshape(28, 28)
    ...
    for i in range(n_gen):
        generated[:, i * 28 : (i + 1) * 28] = np.array(
            results["conj_generated_samples"][i]
        ).reshape(28, 28)
    ax.imshow(generated, cmap="gray")
    ...
    config_text = (
        f"Configuration:\n"
        f"  α_cd = {config['alpha_cd']}\n"
        f"  α_conj = {config['alpha_conj']}\n"
