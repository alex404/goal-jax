...
1. Constructing a Kalman filter from a damped oscillator (standard ``(A, Q, C, R, mu0, Sigma0)`` parameterization).
...
def fit_gradient(
    ...
    init_params = model.initialize(key, shape=0.1)
...
def extract_inference(
    ...
    """Run filtering and smoothing, return per-step (mean, std) of the latent marginals."""
    ...
    smth_means, smth_stds = jax.vmap(extract_mean_std)(smoothed_seq)
...
def main() -> None:
    ...
    init_params = model.initialize(key_train, shape=0.1)
