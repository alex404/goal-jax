...
1. **geometry/**: Core geometric abstractions
   - `algebra/`: Stateless parameter-layout descriptors that never reference `Manifold` (`matrix.py`, `clique.py`, `cut.py`, `util.py`)
   - `manifold/`: Riemannian manifolds, linear maps, embeddings, and combinators. `map.py` holds the map hierarchy, including `SubspaceMap` --- the arity-$n$ linear leaf: a tensor over *selected subspaces* of the manifolds it is multilinear in --- its **factors**, one `LinearEmbedding` each --- read in one matricization. Direction is shape, not indexing: `cod_embs`/`dom_embs` are the two groups and `trn_man` swaps them, so no factor numbers exist anywhere. It is **address-free** --- it knows its factors and what subspace it uses in each, and nothing about where they sit. Its `dom_man`/`cod_man` are `Product`s of its groups' ambient spaces; `Product` lives in `combinators.py` as the tensor-product counterpart to the Cartesian `Tuple` family (dimension a product, not a sum). `clique.py` is the addressing: a clique is a `SubspaceMap` paired with the nodes its factors occupy, and a `LinearCliques` lays a coordinate vector out over its `placements`, one `(members, form)` pair per clique, member $i$ naming the node of factor $i$ in `cod_embs + dom_embs` order; `node_mans` is derived from the forms themselves. Models never write a factor order or an arity: `LevelCliques.cross_placement(rep, {node: emb})` takes the node *set* a coupling touches and derives everything positional --- storage order is those nodes ascending, arity is how many there are, and the output group is the one root node among them. Root forms come from `SubspaceMap.whole`. The one exception is a form borrowed across a level (`graphical/mixture.py`'s $\theta_{XY}$), which arrives address-free and so names its nodes by hand; `cross_paths` bounds what a placement may declare (exactly one root node, sitting first, a single-factor codomain), and a transposed hand-paired form is caught by `node_mans` agreement when the nodes' manifolds differ. Reaching a clique's nodes from a whole partition is a *path*, derived by `LevelCliques.cross_paths`, and `interaction.py`'s `Interaction` sums several path-conjugated subspace maps into one map between whole partitions
...
3. **Maps**: `Map[D, C]` is the generic ABC for parameterized functions between manifolds; concrete `LinearMap[D, C]` leaves are `MatrixMap` and `SquareMap` (matrix-rep backed, acting on the full domain and codomain), `SubspaceMap` (an arity-$n$ tensor over selected subspaces of its factor manifolds, conjugated by per-factor embeddings), and `Interaction` (several path-conjugated cliques summed into one map between whole partitions); `AffineMap` adds a bias, and `MultilayerPerceptron[D, C]` is the nonlinear leaf. `StatefulMap[D, C, S]` is a sibling for stateful (RNN-style) maps.
...
- **Concrete, with fields** when the class is a *value a model builds*. It occurs with multiplicity inside a model and its fields are already primitive --- there is nothing more basic to derive them from: `MatrixMap`, `SquareMap`, `AffineMap`, `MultilayerPerceptron`, `SubspaceMap`, `Interaction`, `CliqueEmbedding`, `RootEmbedding`, `IdentityEmbedding`, `ComposedEmbedding`, `CliqueCut`.

A model is exactly one `LinearCliques`, so it can *be* one; it holds many `SubspaceMap`s, so it builds them. `MatrixMap` is concrete for the same reason `LinearMap` is abstract.
...
| `graphical.py` | `geometry/manifold/clique.py` and `SubspaceMap` from `geometry/manifold/map.py` (the tensor algebra and its readings, incl. vs `MatrixMap` at arity 2; `LinearCliques` placements, `LevelCliques` partitions, `CliqueCut`, `CliqueEmbedding`, `RootEmbedding`) |
| `interaction.py` | `geometry/manifold/interaction.py` (`Interaction` and its `SubspaceMap` vs every live interaction shape, all three operations, incl. arity 3) |
