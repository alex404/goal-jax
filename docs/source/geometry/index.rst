...
The layers build on each other: :doc:`algebra/index` (stateless parameter-layout descriptors) → :doc:`manifold/index` (flat arrays with structure) → :doc:`exponential_family/index` (statistical families with increasing capabilities) → harmoniums and graphical models (composed latent-variable structures).
